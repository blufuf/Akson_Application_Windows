...
class Ui_board4_window(object):
    ...
    def send_wire_temp_add(self):
        number = self.box_1wire_number.currentText()
        sid_text = self.box_1wire_aisid.currentText()

        # Извлекаем только цифры из скобок
        match = re.search(r'\((\d+)\)', sid_text)
        sid = match.group(1) if match else sid_text  # если нет совпадения — отправим как есть
    ...
    def send_di_sets_command(self):
        ...
        sid_text = self.box_disets_sid.currentText()
        mid_text = self.box_disets_mid.currentText()

        match = re.search(r'\((\d+)\)', sid_text)
        sid = match.group(1) if match else sid_text  # если нет совпадения — отправим как есть

        match = re.search(r'\((\d+)\)', mid_text)
        mid = match.group(1) if match else mid_text  # если нет совпадения — отправим как есть
    ...
    def send_ai_sets_command(self):
        ...
        sid_text = self.box_aisets_sid.currentText()
        mid_text = self.box_aisets_mid.currentText()
